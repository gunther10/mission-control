...
export function useWebSocket() {
  ...
  const handleGatewayMessage = useCallback((message: GatewayMessage) => {
    ...
    setConnection({ lastEventAt: observedAt })
      ...
      case 'spawn_result':
        if (message.data?.id) {
          updateSpawnRequest(message.data.id, {
            ...
            sessionId: message.data.sessionId || message.data.sessionKey,
          })
        }
        break
  ...
  }, [setLastMessage, setConnection, setSessions, addLog, updateSpawnRequest, setCronJobs, addTokenUsage])
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        setConnection({
          url: normalizedUrl,
          state: 'connecting',
          reason: 'WebSocket open; completing gateway handshake',
          reconnectAttempts: 0,
          lastEventAt: Date.now(),
        })
      ...
      ws.onclose = (event) => {
        ...
        if (manualDisconnectRef.current) {
          setConnection({ state: 'disconnected', reason: 'Disconnected manually', reconnectAttempts: 0, nextExpectedAt: undefined })
          return
        }
        // Skip auto-reconnect for non-retryable handshake failures
        if (nonRetryableErrorRef.current) {
          setConnection({ state: 'blocked', reconnectAttempts: 0, nextExpectedAt: undefined })
          return
        }
        ...
        if (attempts < maxReconnectAttempts) {
          ...
          setConnection({
            state: 'reconnecting',
            reason: event.reason || 'Connection dropped; retrying automatically',
            reconnectAttempts: attempts + 1,
            nextExpectedAt: Date.now() + timeout,
          })
          ...
          setConnection({
            ...
            actionRequired: 'Reconnect manually',
            nextExpectedAt: undefined,
          })
        }
      }
