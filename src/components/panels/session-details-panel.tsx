...
export function SessionDetailsPanel() {
  ...
  const waitingOnYouCount = sortedSessions.filter((session) => {
    const status = deriveSessionOperatorStatus({ session, connection, execApprovals, spawnRequests, cronJobs })
    return status.state === 'waiting_for_human' || status.state === 'waiting_for_approval'
  }).length
  const blockedCount = sortedSessions.filter((session) => {
    const status = deriveSessionOperatorStatus({ session, connection, execApprovals, spawnRequests, cronJobs })
    return status.state === 'blocked' || status.state === 'failed' || status.state === 'reconnecting'
  }).length
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid md:grid-cols-4 gap-3">
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Gateway</div>
          <div className={`inline-flex px-2 py-1 rounded-full border text-xs font-medium ${getOperatorToneClasses(connectionStatus.tone)}`}>
            {connectionStatus.label}
          </div>
          <div className="mt-2 text-sm text-muted-foreground">{connectionStatus.reason}</div>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Running</div>
          <div className="text-2xl font-semibold text-foreground">{sortedSessions.filter(s => s.active).length}</div>
          <div className="text-sm text-muted-foreground">sessions with live activity</div>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Waiting on you</div>
          <div className="text-2xl font-semibold text-foreground">{waitingOnYouCount}</div>
          <div className="text-sm text-muted-foreground">includes reply + approval waits</div>
        </div>
        <div className="bg-card border border-border rounded-lg p-4">
          <div className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Blocked</div>
          <div className="text-2xl font-semibold text-foreground">{blockedCount}</div>
          <div className="text-sm text-muted-foreground">failed, blocked, or reconnecting</div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {sortedSessions.length === 0 ? (
            ...
            sortedSessions.map((session) => {
              ...
              const operatorStatus = deriveSessionOperatorStatus({ session, connection, execApprovals, spawnRequests, cronJobs })
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                              ...
                              <span className={`px-2 py-0.5 rounded-full border text-xs ${getOperatorToneClasses(operatorStatus.tone)}`}>
                    ...
                    <div className="rounded-lg border border-border/60 bg-secondary/30 p-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        ...
                        {operatorStatus.nextExpectedAt && (
                          <span>Next expected: {formatNextExpectedTime(operatorStatus.nextExpectedAt)}</span>
                        )}
                        {operatorStatus.actionRequired && (
                          <span>Action required: {operatorStatus.actionRequired}</span>
                        )}
                      </div>
                    </div>
