...
export function TaskBoardPanel() {
  ...
  const [quickDraftProjectId, setQuickDraftProjectId] = useState<string>(activeProject ? String(activeProject.id) : '')
    ...
    try {
      const response = await fetch('/api/tasks', {
        ...
        body: JSON.stringify({
          ...
          project_id: quickDraftProjectId ? Number(quickDraftProjectId) : undefined,
          status: 'inbox',
          priority: 'medium',
          metadata: {
            intake_mode: 'quick-draft',
            assumptions: {
              project_inferred: Boolean(quickDraftProjectId),
            },
          },
        }),
      })
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || data.details?.join(', ') || 'Failed to create draft task')
      }

      setQuickDraftTitle('')
      await fetchData()
    } catch (err) {
      setQuickDraftError(err instanceof Error ? err.message : 'Failed to create draft task')
    } finally {
      setQuickDraftSaving(false)
    }
  }, [fetchData, quickDraftProjectId, quickDraftTitle])
  ...
  useEffect(() => {
    if (activeProject?.id) {
      setQuickDraftProjectId(String(activeProject.id))
      return
    }
    if (!quickDraftProjectId && projects[0]?.id) {
      setQuickDraftProjectId(String(projects[0].id))
    }
  }, [activeProject, projects, quickDraftProjectId])
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="border-b border-border bg-surface-0/70 p-4">
        <div className="grid gap-3 lg:grid-cols-[1fr_220px_auto]">
          <div>
            <label className="block text-xs uppercase tracking-wide text-muted-foreground mb-1">Quick draft task</label>
            <input
              ...
              placeholder="Write the task in plain language. I’ll create it as a draft."
              className="w-full h-10 px-3 bg-background text-foreground border border-border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
            />
            <p className="mt-1 text-[11px] text-muted-foreground">Fast path: title only, inbox status, defaults filled in later.</p>
            {quickDraftError && <p className="mt-1 text-[11px] text-red-400">{quickDraftError}</p>}
          </div>
          <div>
            <label className="block text-xs uppercase tracking-wide text-muted-foreground mb-1">Project</label>
            <select
              value={quickDraftProjectId}
              onChange={(e) => setQuickDraftProjectId(e.target.value)}
              className="w-full h-10 px-3 bg-background text-foreground border border-border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
            >
          ...
          <div className="flex items-end">
            <Button onClick={() => void createDraftTask()} disabled={quickDraftSaving || !quickDraftTitle.trim()} className="w-full lg:w-auto">
              {quickDraftSaving ? 'Creating…' : 'Create Draft'}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 px-4 pb-4 md:grid-cols-4">
        <div className="rounded-lg border border-border bg-surface-0 px-3 py-2">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Waiting for review</div>
          <div className="text-xl font-semibold text-foreground">{tasks.filter(task => task.status === 'review' || task.status === 'quality_review').length}</div>
          <div className="text-xs text-muted-foreground">needs Henrique to look</div>
        </div>
        <div className="rounded-lg border border-border bg-surface-0 px-3 py-2">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Live execution</div>
          <div className="text-xl font-semibold text-foreground">{tasks.filter(task => deriveTaskOperatorStatus({ task: task as any, sessions: sessions as any, connection, execApprovals, spawnRequests, cronJobs }).state === 'running').length}</div>
          <div className="text-xs text-muted-foreground">tasks backed by active sessions</div>
        </div>
        <div className="rounded-lg border border-border bg-surface-0 px-3 py-2">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Blocked</div>
          <div className="text-xl font-semibold text-foreground">{tasks.filter(task => {
            const state = deriveTaskOperatorStatus({ task: task as any, sessions: sessions as any, connection, execApprovals, spawnRequests, cronJobs }).state
            return state === 'blocked' || state === 'failed' || state === 'reconnecting'
          }).length}</div>
          <div className="text-xs text-muted-foreground">tasks that need intervention</div>
        </div>
        <div className="rounded-lg border border-border bg-surface-0 px-3 py-2">
          <div className="text-[10px] uppercase tracking-wide text-muted-foreground mb-1">Waiting on you</div>
          <div className="text-xl font-semibold text-foreground">{tasks.filter(task => {
            const state = deriveTaskOperatorStatus({ task: task as any, sessions: sessions as any, connection, execApprovals, spawnRequests, cronJobs }).state
            return state === 'waiting_for_human' || state === 'waiting_for_approval'
          }).length}</div>
          <div className="text-xs text-muted-foreground">approval or review blockers</div>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 p-2.5 space-y-2.5 min-h-32 overflow-y-auto">
              {tasksByStatus[column.key]?.map(task => {
                const operatorStatus = deriveTaskOperatorStatus({
                  task: task as any,
                ...
                >
                  ...
                  <div className="mt-2 ml-5.5 rounded-md border border-border/50 bg-background/40 px-2.5 py-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`px-2 py-0.5 rounded-full border text-[10px] font-medium ${getOperatorToneClasses(operatorStatus.tone)}`}>
                        {operatorStatus.label}
                      </span>
                      <span className="text-[11px] text-muted-foreground">{operatorStatus.reason}</span>
                    </div>
                    <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-muted-foreground/80">
                      <span>Progress: {formatRelativeOperatorTime(operatorStatus.lastProgressAt)}</span>
                      {operatorStatus.nextExpectedAt && <span>Next: {formatNextExpectedTime(operatorStatus.nextExpectedAt)}</span>}
                    </div>
                  </div>
...
function TaskDetailModal({
  ...
  const router = useRouter()
  const { currentUser, sessions, connection, execApprovals, spawnRequests, cronJobs } = useMissionControl()
          ...
          {activeTab === 'details' && (
            <div id="tabpanel-details" role="tabpanel" aria-label="Details" className="mt-4 space-y-4">
              <div className="rounded-lg border border-border/60 bg-secondary/30 p-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`px-2 py-1 rounded-full border text-xs font-medium ${getOperatorToneClasses(operatorStatus.tone)}`}>
                    {operatorStatus.label}
                  </span>
                  <span className="text-sm text-foreground">{operatorStatus.reason}</span>
                </div>
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                  <span>Last progress: {formatRelativeOperatorTime(operatorStatus.lastProgressAt)}</span>
                  {operatorStatus.nextExpectedAt && <span>Next expected: {formatNextExpectedTime(operatorStatus.nextExpectedAt)}</span>}
                  {operatorStatus.actionRequired && <span>Action required: {operatorStatus.actionRequired}</span>}
                </div>
              </div>
...
function CreateTaskModal({
  ...
  const [formData, setFormData] = useState({
    ...
    project_id: defaultProjectId ? String(defaultProjectId) : (projects[0]?.id ? String(projects[0].id) : ''),
    assigned_to: '',
    tags: '',
  })
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="create-task-title" className="bg-card border border-border rounded-lg max-w-md w-full">
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mb-4 rounded-lg border border-border/60 bg-secondary/30 p-3 text-sm text-muted-foreground">
            Draft-first flow: create the task now, fill in owner/details later. Only title is required.
          </div>
          ...
          <div className="flex gap-3 mt-6">
            <Button type="submit" className="flex-1" disabled={isRecurring && !parsedSchedule}>
              {isRecurring ? 'Create Recurring Task' : 'Create Draft Task'}
            </Button>
