...
export function ProjectManagerModal({
  ...
  const [projectTaskDrafts, setProjectTaskDrafts] = useState<Record<number, string>>({})
  const [creatingTaskForProject, setCreatingTaskForProject] = useState<number | null>(null)
  ...
  const createTaskForProject = async (project: Project) => {
    ...
    try {
      ...
      const response = await fetch('/api/tasks', {
        ...
        body: JSON.stringify({
          ...
          priority: 'medium',
          metadata: {
            intake_mode: 'project-manager-quick-add',
            assumptions: {
              project_locked: true,
            },
          },
                  ...
                  {editingId === project.id && (
                    <div className="border-t border-border p-3 bg-surface-1/50 space-y-3" onClick={(e) => e.stopPropagation()}>
                      <div className="rounded-lg border border-border/60 bg-card/60 p-3">
                        <div className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Quick add task to {project.name}</div>
                        <div className="flex gap-2">
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                e.preventDefault()
                                void createTaskForProject(project)
                              }
                            }}
                            placeholder="Write the task in plain language"
                            className="flex-1 bg-surface-1 text-foreground border border-border rounded-md px-3 py-2 text-sm"
                          />
                          <Button
                            size="sm"
                            onClick={() => void createTaskForProject(project)}
                            disabled={creatingTaskForProject === project.id || !(projectTaskDrafts[project.id] || '').trim()}
                          >
                            {creatingTaskForProject === project.id ? 'Creating…' : 'Create Draft'}
                          </Button>
                        </div>
                        <p className="mt-2 text-[11px] text-muted-foreground">No extra questions. This creates an inbox task under the current project.</p>
                      </div>
